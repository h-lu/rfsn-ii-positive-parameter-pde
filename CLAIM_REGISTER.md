...
| V5 | The future-staying hypersurface in V4 matches through the intermediate chart to the central algebraic sheet with two parameter derivatives and action-cut covariance. | Proved | [Theorem V5, global future-staying tube, endpoint adjoint, and matching operator](van-der-pol/CENTRAL_OUTER_MATCHING.md) |
| V5A | The matched positive-parameter algebraic end has reference-normalized physical length and action finite parts with mixed two-jets, admissible-coordinate covariance, and strict finite-branch composition. | Proved | [Theorem V5A, fixed-\(Q\) flat shadowing and reference-tail subtraction](van-der-pol/OUTER_ALGEBRAIC_FINITE_PART.md) |
| V6 | The positive-parameter van der Pol system has an exhaustive high-winding return/first-exit relation with two compatible action finite parts. | Proved | [Theorem V6 and the whole-cell pullback, stratification, and exact-action proof](van-der-pol/TWO_END_RETURN_EXIT_AND_PDE.md), using the strictly bounded [frozen modular import](van-der-pol/RETURN_EXIT_CODING_IMPORT.md) |
| V7 | Bounded symbolic itineraries in V6 yield stationary periodic, multipulse, or aperiodic spatial PDE patterns. | Proved | [Theorem V7, completed-section coding, and exact inverse PDE scaling](van-der-pol/TWO_END_RETURN_EXIT_AND_PDE.md), using only the graph proved exhaustive in V6 |
