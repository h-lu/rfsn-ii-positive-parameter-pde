...
\(|b|\le27/200000=1.35\times10^{-4}\) from the rigorously narrowed normal tube
...
the first final non-PASS.  A valid mathematical `INCONCLUSIVE` never enters
the fallback route.  Completed atomic cells are resumed; an unfinished cell
may be rerun after interruption.  `verify` requires exactly 393,216 unique
`PASS` cells with the frozen grid, graph contract, gate inventory, execution
provenance, and artifact hashes, then reports only the critical worst-case
extrema.  No full cover is asserted merely because the driver was prepared.
...
A `V4.OUTER_GRAPH` local mathematical `PASS` supports, independently
of the P2 chain, only:

- the regular positive compactified energy root and the explicit
  $|E|\le10^{-3}$, $0\le z\le2/9$,
...
It supplies no V5 incidence or exchange, matched source orbit, outer action
finite part, aggregate release status, or independent replay. The
reproducible source, exact cover, and numerical margins are recorded in
[`V4_OUTER_GRAPH_REPORT.md`](V4_OUTER_GRAPH_REPORT.md).
