...
child, `V2.CHART.ANALYTIC_NORMAL_FORM`, now also passes locally from the
  proved all-orders majorant and its bound source checker.  The third child,
  `V2.CHART.ZERO_ENERGY`, passes locally from the transferred action majorant,
  strict rational Krawczyk gate, and explicit mixed-jet/Cauchy bounds.  The
  fourth child, `V2.CHART.EXACT_SECTIONS`, passes locally from the frozen
  radial block, authenticated arbitrary-\(q\) identities, fixed gauges, and
  exact action preservation.  The fifth child,
  `V2.CHART.WEIGHTED_PASSAGE`, passes locally from the signed radial laws,
  absolute Kato deck, all-finite-log-order generator, and uniform clock
  inversion.  The sixth child, `V2.CHART.PHYSICAL_SLIDES`, passes locally from
  the inherited physical faces, finite first-hit slides, complete regularity
  rectangle, and (D12) with \(C_{\rm phys}=7\).  These aggregates remain
  non-claim-bearing because independent replay is 1/2.  The remaining overlap
  child, P2e,
