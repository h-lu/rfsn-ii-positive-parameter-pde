...
| V2D-I | P2d proof-interface and Kato sign clarification: the positive Kato frame uses \(k_2=\mathfrak J_uk_1\); the full conjugation \(\mathcal T=\operatorname{diag}(C_0,C_0)\) is symplectic and satisfies \(I_2^{\rm F}(\mathcal Tz)=I_2^{\rm K}(z)\), so it preserves the action sign, while direct quadrature on the explicit Kato radial sections gives the phase term \(-\beta\alpha^{-1}\log|\nu|\), the residual offset \(\widetilde b^{\rm K}=b^{\rm K}-\beta t^{\rm K}\), and the Kato limiting template; [`P2_VALIDATION_CONTRACT.md`](../validation/rigorous/P2_VALIDATION_CONTRACT.md), Section 5.5, [`CENTRAL_CONTINUATION.md`](../van-der-pol/CENTRAL_CONTINUATION.md), item V2(3), and [`RETURN_EXIT_CODING_IMPORT.md`](../van-der-pol/RETURN_EXIT_CODING_IMPORT.md), equation (3K) | `LOCAL-AMENDMENT / Derived + locally validated frame atom` | V1 exact symplectic sign convention plus the P2bK normalized Kato frame, the 59-check exact audit, and the strict 512-cell parameter-two-jet probe establish local mathematical `PASS` for `V2.CHART.SYMPLECTIC_FRAME`; the nonlinear analytic normal form, zero-energy branch, exact nonlinear sections, weighted passage, physical slides, overlaps, and the parent `V2.EXACT_CHART` remain pending | None |
| V2D-NF | Proposed van-der-Pol-specific global Moser majorant on the P2d Kato chart, with an exact \(q=1,2\) Lie prefix, fixed nested complex domains, inverse domains, and primitive gauge; [`EXPLICIT_GLOBAL_MOSER_MAJORANT.md`](EXPLICIT_GLOBAL_MOSER_MAJORANT.md), [`audit_p2d_normal_form_exact.py`](../validation/rigorous/audit_p2d_normal_form_exact.py), [`p2d_normal_form_scout.py`](../validation/rigorous/design/p2d_normal_form_scout.py), and [`P2D_NORMAL_FORM_DESIGN_REPORT.md`](../validation/rigorous/P2D_NORMAL_FORM_DESIGN_REPORT.md) | `LOCAL-AMENDMENT / Proposed` | V1, V2D-I, and the archived local pass for `V2.CHART.SYMPLECTIC_FRAME`; the 26-check exact audit establishes only the finite \(q=1,2\) algebra and, at \(r=0\), \(Z_4=((I_2^{\rm K})^2-I_1^2)/120\).  Conditional on continuation of a formal zero-energy action graph \(I_1=-\nu+c_2\nu^2+\cdots\), coefficient comparison gives \(c_2=0\); the audit constructs no such full graph.  The authenticated scout passes candidate input and domain gates only.  The all-orders \(J^2\) recurrence, convergence, and map/inverse/primitive parameter-\(C^2\) tails remain unproved, so `V2.CHART.ANALYTIC_NORMAL_FORM` and `V2.EXACT_CHART` remain `OPEN`. | None |
...
`V2.CHART.SYMPLECTIC_FRAME`, also has a local mathematical pass from the
P2bK prerequisite, exact audit, and strict interval frame probe.  These
aggregates remain non-claim-bearing because independent replay is 1/2.  The
remaining six P2d children, P2e, and P3--P5 remain pending.
