...
| Explicit-box validation (#7) | v1 mathematical `FAIL`; v2 P1, restricted P2a--P2d, and full-energy `V4.OUTER_GRAPH` local mathematical `PASS`; three P2e phase gaps and the zero-action source-chart criterion local `PASS`; the POLE zero-action first-hit bridge has a complete computed-interval design `PASS`; one representative v2 cell has strict true-source/V5-graph incidence `PASS`, and the grouped candidate-hull kernel passes on three disclosed cells as a non-claim-bearing feasibility check; aggregate `INCONCLUSIVE`, `claim_bearing=false` | The immutable v1 box is \([0.04,0.08]\times[-0.25,0.25]\times[0.8,1.2]\); its strict P2e phase-order failure remains the verdict for v1. On v2, the true \(\nu=0\) source has a nonsingular exact zero-energy enclosure, and a complete strict axis skeleton would thicken to some positive uniform action subcollar. The POLE first hit is covered on all 4096 bridge cells. The 16,384-cell V4 cover proves the full corridor \(|E|\le10^{-3}\), \(0\le z\le2/9\), including the \(z\)-location of the parameter-dependent \(R=2\) cut. The V5 result uses the expanded finite-\(K_1\) base \(|b|\le27/200000=1.35\times10^{-4}\), the rigorously narrowed \(|n|\le1/12500=8\times10^{-5}\) tube, and parameterized interval Newton to prove the incidence mechanism. The complete phase scan now uses eight slabs instead of the earlier sixteen, halving the ordinary slab scan. Each lower, centre, and upper sample has 12 candidates and 12 exterior propagations, so these data do not claim that grouping itself reduces the exterior count; the three samples also do not constitute the complete parameter cover. A frozen \(64\times128\times48\) full-cover configuration and driver are prepared, but no run manifest has yet been created and no full cover is counted as evidence. A fixed-label ALG finite-gate corner cover remains honestly `INCONCLUSIVE`. The uniform joint incidence and exhaustive census, \(m_{\rm ax}\), transported traces, remaining V5 matching, and P3--P5 interval obligations remain pending; the whole fixed \(2^{-55}\) disk is not claimed. See the [V5 incidence proof](validation/rigorous/V5_SOURCE_INCIDENCE_REPORT.md), [V4 outer-graph proof](validation/rigorous/V4_OUTER_GRAPH_REPORT.md), [ALG finite-gate obstruction](validation/rigorous/P2E_ALG_FIXED_PHASE_OBSTRUCTION_REPORT.md), [pole-axis cover](validation/rigorous/P2E_POLE_AXIS_FIRST_HIT_COVER_REPORT.md), [axis-chart report](validation/rigorous/P2E_AXIS_CHART_REPORT.md), and [v2 phase report](validation/rigorous/P2E_V2_PHASE_ORDER_REPORT.md). |
