...
length and action.  A resolved central--outer graph construction transports
this hypersurface back to a fixed central section.  A transverse phase--time
incidence then selects a true source orbit through the algebraic-directed gate
and onto that sheet.  Combining these constructions
...
Figure~\ref{fig:computed-profiles} gives a separate, nonrigorous realization
of the stationary objects in the original PDE variables.  It uses the single
exploratory parameter point
$(r,a_2,\epsilon)=(0.08,0,1)$, equivalently
$(d,a,\epsilon)=(4.096\times10^{-5},1,1)$.  The maximum saved finite-difference
stationary residual is $6.81\times10^{-7}$ for the periodic profiles and
$1.28\times10^{-6}$ for the one- through four-pulse profiles.  These
computations illustrate the output of the theorem chain, but the parameter
point has not been placed in a certified explicit existence box, and the
numerical A/B and pulse-count labels have not been identified with exact
edges or words of the marked V6--V7 relation.

\begin{figure}[t]
 \centering
 \includegraphics[width=\textwidth]{computed_stationary_profiles.pdf}
 \caption{\textbf{COMPUTED/E1.}  Finite-domain numerical
 candidates for stationary profiles of the original van der Pol PDE at
 $(r,a_2,\epsilon)=(0.08,0,1)$.  (a) Two full-ODE periodic candidates,
 distinguished by both color and line style.  (b) Full-ODE one- through
 four-pulse homoclinic candidates reconstructed in the physical coordinate.
 All curves show $u-a$; the dotted horizontal line is the homogeneous state.
 The periodic candidates have closure, energy, step-refinement, and
 stationary-residual checks; the localized candidates have collocation,
 boundary, tail, energy, and stationary-residual checks, all archived with
 the numerical atlas.  The family and pulse-count labels in this panel
 are numerical labels, not certified V6/V7 edge itineraries.  The figure is
 non-claim-bearing and makes no temporal-stability, Turing-selection, or
 canard claim.}
 \label{fig:computed-profiles}
\end{figure}
...
transform constructs the algebraic future near infinity.  A resolved $K_1$
graph argument transports that same future-staying sheet backward to a fixed
central section, but this produces only a target graph, not an orbit from the
source.  The missing connection is a phase--time matching equation whose
determinant factors into the nonzero section speed and the nonzero
source-phase incidence.  Finally, replacing the two finite gates inside their
...
The algebraic terminal requires two geometric constructions and one
incidence argument.  We first characterize the tails that remain in a
compactified outer corridor.  We then transport this same future-staying
hypersurface backward through the resolved $K_1$ corner to a fixed central
section.  Finally, a phase--time matching argument proves that the continued
true unstable source meets the transported graph transversely.  Thus graph
transport determines the future of the matched points, whereas source
incidence proves that the selected algebraic component is actually reached.
...
Proposition~\ref{prop:outer-graph} does not imply that a true-source orbit
through the finite algebraic-directed gate of Proposition~\ref{prop:central}
continues on \eqref{eq:outer-graph}.  The gate lives in the universal central chart
...
The two remaining arrows must be kept separate:
\[
 \begin{aligned}
  \text{outer sheet}
    &\xrightarrow{\text{resolved }K_1\text{ graph transport}}
      \text{central target graph},\\
  \text{source flight}
    &\xrightarrow{\text{phase--time incidence}}
      \text{central target graph}.
 \end{aligned}
\]
Graph transport identifies the future of every point on the target graph;
the incidence equation below proves that a true source orbit reaches it.

\begin{proposition}[Central--outer graph and source matching]\label{prop:matching}
...
\begin{equation}\label{eq:matching-operator}
 ...
 \end{pmatrix}.
\end{equation}
A zero of $\mathfrak M_\mu$ is precisely an intersection of the true source
flight with the transported outer graph on the chosen central section.  At
such a zero, exact row reduction gives
...
\end{equation}
Here $s_\mu\ne0$ is the transverse section speed, whereas $\chi_\mu\ne0$
is the incidence of the source-phase direction with the matched graph inside
that section.  Neither factor is the Jost exchange pairing
$\chi_{{\rm ex},\mu}$.  The frozen operator has the certified zero from
${\rm(H_{Jost})}$, and its continuations satisfy
$|s_\mu|\ge s_*>0$ and
$|\chi_\mu-\chi_0|<|\chi_0|/2$.  Thus the phase/time operator is uniformly
invertible and selects a unique zero in the fixed matching neighborhood;
this zero depends $C^2$ on $\mu$.  Its orbit crosses the original finite gate; because the
target graph is the backward saturation of the outer future-staying sheet,
invariance carries this same orbit to the algebraic end.  Every truncated
matching action satisfies exact moving-cut covariance.
\end{proposition}

\begin{proof}
\emph{Graph transport.}
...
\emph{True-source incidence.}
Separately, the normalized central target row converges to $Dg_{\rm a}$, and
the source flight converges in $C^1$.  Therefore
...
Propositions~\ref{prop:pole} and \ref{prop:matching} identify the selected
terminal futures associated with those gates.  Because both replacements take place inside protected flow boxes
...
carrier keeps its original hit time.  The phase--time zero in
Proposition~\ref{prop:matching} supplies the selected anchored point of this
subdivision, while the intrinsic target row supplies its nonzero conormal.
Thus the selected algebraic component is nonempty and transverse without
introducing a new first-hit time.  Inside the protected pole outcome, an
